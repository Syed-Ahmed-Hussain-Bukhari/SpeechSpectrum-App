...
class SpeechController extends GetxController {
  ...
  Future<String?> submitSpeechAndGetId({
    ...
    try {
      ...
      final msg = prediction != null
          ? 'Submitted! Risk: ${prediction.riskInterpretation} '
              '(${prediction.saScore.toStringAsFixed(1)}/${prediction.maxScore})'
          : 'Speech recording submitted successfully!';
      ...
      // Pre-populate currentSubmission from the inline result so detail
      // screen shows data instantly without waiting for a second GET call.
      currentSubmission.value =
          _buildDetailFromCreateResponse(response.data);
  ...
  /// Legacy helper kept for any callers that only need a bool.
  Future<bool> submitSpeech({
    required File audioFile,
    required int durationSeconds,
    required String format,
  ...
  /// Converts a POST response into a [SpeechSubmissionDetail] for the cache.
  SpeechSubmissionDetail _buildDetailFromCreateResponse(
      SpeechSubmissionWithResult data) {
    final List<SpeechResult> results = [];

    // Prefer speech_result (has result sub-object) over prediction_result
    if (data.speechResult != null) {
      results.add(data.speechResult!);
    } else if (data.predictionResult != null) {
      // Synthesise a SpeechResult from the prediction_result
      final p = data.predictionResult!;
      results.add(SpeechResult(
        speechResultId: '',
        speechSubmissionId: data.speechSubmissionId,
        result: AnalysisResult(
          riskInterpretation: p.riskInterpretation,
          severityScore: p.severityScore,
          maxScore: p.maxScore,
          saScore: p.saScore,
          severityPercentage: p.severityPercentage,
          riskLevel: p.riskLevel,
          modelType: p.modelType,
          filename: p.filename,
        ),
        generatedAt: data.submittedAt,
      ));
    }
