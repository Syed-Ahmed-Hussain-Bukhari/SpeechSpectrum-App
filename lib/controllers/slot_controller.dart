...
class SlotController extends GetxController {
    ...
    try {
      ...
      if (needsZoom) {
        try {
          ...
          debugPrint('⚠️ Zoom link generation failed (non-fatal): $e');
        }
      }

      // ── FIXED: only send fees relevant to selected mode ──
      final mode = selectedMode.value;
      double? feeOnline;
      double? feePhysical;

      if (mode == 'online' || mode == 'both') {
        feeOnline = double.tryParse(feeOnlineCtrl.text.trim());
      }
      if (mode == 'physical' || mode == 'both') {
        feePhysical = double.tryParse(feePhysicalCtrl.text.trim());
      }
      ...
      // ── FIXED: show friendly duplicate error ──
      if (msg.toLowerCase().contains('duplicate') ||
          msg.toLowerCase().contains('overlap') ||
          msg.toLowerCase().contains('no_overlap')) {
        _showDuplicateError();
      } else {
        _showError(msg);
      }
  ...
  // ── Show friendly duplicate / overlap error ────────────────
          ...
          children: [
            Container(
              ...
              child: const Icon(Icons.event_busy_rounded,
                  color: AppColors.warningColor, size: 34),
            ),
            const SizedBox(height: 14),
            const Text('Time Slot Conflict',
                style: TextStyle(
                    fontSize: 17,
            ...
            const Text(
              'A slot already exists that overlaps with the selected date and time. '
              'Please choose a different time or date.',
              textAlign: TextAlign.center,
              style: TextStyle(fontSize: 13, color: AppColors.textSecondaryColor, height: 1.5),
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: const Text('Change Time / Date',
  ...
  bool _validateForm() {
    ...
    final mode = selectedMode.value;
    if (mode != 'physical') {
      if (feeOnlineCtrl.text.trim().isEmpty ||
          double.tryParse(feeOnlineCtrl.text.trim()) == null) {
        _showError('Please enter a valid online fee');
        return false;
      }
    ...
    if (mode != 'online') {
      if (feePhysicalCtrl.text.trim().isEmpty ||
          double.tryParse(feePhysicalCtrl.text.trim()) == null) {
        _showError('Please enter a valid physical fee');
        return false;
      }
