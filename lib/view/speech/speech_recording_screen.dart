...
class _SpeechRecordingScreenState extends State<SpeechRecordingScreen>
  ...
  Future<void> _pickAudioFile() async {
    try {
      final result = await FilePicker.platform.pickFiles(
        type: FileType.custom,
        allowedExtensions: ['wav'], // ✅ WAV only
        allowMultiple: false,
      );
      ...
      // Extra guard: reject anything that slipped through that isn't .wav
      final ext = pickedFile.name.split('.').last.toLowerCase();
      if (ext != 'wav') {
        Get.snackbar(
          'Invalid Format',
          'Only .wav files are accepted. Please select a WAV audio file.',
          backgroundColor: AppColors.errorColor,
          colorText: AppColors.whiteColor,
          snackPosition: SnackPosition.TOP,
          duration: const Duration(seconds: 4),
      ...
      // ── Estimate duration from file size (WAV: 44.1kHz 16bit mono ≈ 88 KB/s)
      final fileSizeBytes = await file.length();
      final int estimatedDuration =
          (fileSizeBytes / 88200).floor(); // WAV only now
      ...
      debugPrint(
          '✅ WAV file picked: $fileName (est. ${estimatedDuration}s)');
      ...
      Get.snackbar(
        'Error',
        'Failed to pick audio file. Make sure the file is a valid WAV file.',
        backgroundColor: AppColors.errorColor,
        colorText: AppColors.whiteColor,
        snackPosition: SnackPosition.TOP,
      );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (isUpload) ...[
            _instruction(context,
                '2. Tap the upload area to pick a WAV file'),
            _instruction(context,
                '3. File must be at least $_minDurationSeconds seconds long'),
            _instruction(context,
                '4. Only .wav format is accepted'),         // ✅ updated
            _instruction(context,
                '5. Tap "Upload & Analyse" to submit'),
          ] else ...[
            _instruction(context, '2. Find a quiet environment'),
