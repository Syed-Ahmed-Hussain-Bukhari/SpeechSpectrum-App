...
//   static const _tabs = [
//     ('All', Icons.list_alt_rounded),
//     ('Requested', Icons.pending_actions_rounded),
//     ('Confirmed', Icons.check_circle_outline_rounded),
...
//       .toList()
//     ..sort((a, b) => _parseDate(b.scheduledAt)
//         .compareTo(_parseDate(a.scheduledAt)));
...
//                 emptyMessage: 'No expired appointments',
//                 emptySubMessage:
//                     'Past scheduled/confirmed sessions appear here'),
...
//           bool effectiveShowPayment;
//           bool effectiveShowMeetLink;
//           bool effectiveShowChildProfile;

//           if (isExpiredTab) {
//             effectiveShowPayment = appt.isConfirmed;
//             effectiveShowMeetLink = false;
//             effectiveShowChildProfile = false;
//           } else if (smartMode) {
//             effectiveShowPayment = appt.isConfirmed && !expired;
//             effectiveShowMeetLink = appt.isConfirmed && !expired;
//             effectiveShowChildProfile =
//                 (appt.isScheduled || appt.isConfirmed) && !expired;
//           } else {
//             effectiveShowPayment = showPayment;
//             effectiveShowMeetLink = showMeetLink && !expired;
//             effectiveShowChildProfile = showChildProfileBtn && !expired;
//           }
...
//             showPayment: effectiveShowPayment,
//             showMeetLink: effectiveShowMeetLink,
//             showChildProfileBtn: effectiveShowChildProfile,
//             isExpired: expired,
//             isExpiredTab: isExpiredTab,
//             size: size,
//           );
...
//   final bool showPayment;
//   final bool showMeetLink;
//   final bool showChildProfileBtn;
//   final bool isExpired;
//   final bool isExpiredTab;
//   final CustomSize size;
...
//     required this.showPayment,
//     required this.showMeetLink,
//     required this.showChildProfileBtn,
//     required this.isExpired,
//     required this.isExpiredTab,
//     required this.size,
//   });
...
//                     children: [
//                       if (showChildProfileBtn &&
//                           appt.childId.isNotEmpty)
//                         GestureDetector(
//                           onTap: () {
//                             Get.toNamed(
...
//     // Child profile button visible for scheduled (requested) & confirmed only, NOT expired
//     final bool canViewChildProfile =
//         (appt.isScheduled || appt.isConfirmed) && !expired;
//     final String childId = appt.childId;
//     final String childName = appt.children?.childName ?? '';
...
//               // ── Child Profile Button (requested & confirmed, not expired) ──
//               if (canViewChildProfile && childId.isNotEmpty) ...[
//                 _childProfileButton(context, size, childId, childName),
//                 SizedBox(height: size.customHeight(context) * 0.018),
//               ],
...
//               // ── CONFIRMED + EXPIRED: Meet link expired ──────────────────
//               if (isConfirmed && expired &&
//                   appt.meetLink != null &&
//                   appt.meetLink!.isNotEmpty) ...[
//                 _meetLinkExpiredCard(context, size),
...
//   Widget _childProfileButton(BuildContext context, CustomSize size,
...
//           ],
//           border: Border.all(
//               color: AppColors.secondaryColor.withOpacity(0.3), width: 1.5),
//         ),
//         child: Row(
//           children: [
//             Container(
...
//                   ),
//                   Text(
//                     'Overview · Health · Screening · Speech',
//                     style: GoogleFonts.poppins(
//                         fontSize: 11,
//                         color: AppColors.textSecondaryColor),
//                   ),
...
//                   const SizedBox(width: 4),
//                   const Icon(Icons.arrow_forward_ios_rounded,
//                       size: 11, color: AppColors.secondaryColor),
//                 ],
//               ),
//             ),
//           ],
...
class _ParentMyAppointmentsScreenState
  ...
  // ── Recent detection: created_at within last 7 days ───────
  bool _isRecent(MyAppointmentItem appt) {
    try {
      final created = DateTime.parse(appt.createdAt).toLocal();
      ...
      body: Obx(() {
        ...
        return TabBarView(
          ...
          children: [
            // All
            _buildSectionedList(
              context, size, _allList,
              showChildProfileBtn: true,
              isExpiredTab: false,
            ),
            // Requested
            _buildSectionedList(
              context, size, _requestedList,
              showChildProfileBtn: true,
              isExpiredTab: false,
              emptyMessage: 'No pending requests',
              emptySubMessage: 'Requested appointments will appear here',
            ...
            // Completed
            _buildSectionedList(
              context, size, _c.completedList,
              isExpiredTab: false,
            ),
            // Cancelled
            _buildSectionedList(
              context, size, _c.cancelledList,
              isExpiredTab: false,
            ),
            // No Show
            _buildSectionedList(
              context, size, _c.noShowList,
              isExpiredTab: false,
            ),
          ],
        );
  ...
  // ══════════════════════════════════════════════════════════════
  // UNIFIED SECTIONED LIST — Recent + Older for ALL tabs
  // ══════════════════════════════════════════════════════════════
  Widget _buildSectionedList(
    ...
    if (list.isEmpty) {
      return isExpiredTab
          ? _buildEmptyExpired(context, size)
          : _buildEmpty(context, size,
              message: emptyMessage, subMessage: emptySubMessage);
    }

    // Sort by created_at descending (newest first)
    final sorted = List<MyAppointmentItem>.from(list)
      ..sort((a, b) {
        try {
    ...
    final recentList = sorted.where(_isRecent).toList();
    final olderList = sorted.where((a) => !_isRecent(a)).toList();

    return RefreshIndicator(
      color: isExpiredTab ? const Color(0xFFE65100) : AppColors.primaryColor,
      onRefresh: _c.fetchMyAppointments,
      child: ListView(
        padding: EdgeInsets.fromLTRB(
          size.customWidth(context) * 0.045,
        ...
        children: [
          // ── Expired banner (Expired tab only) ───────────────
          if (isExpiredTab) ...[
            _buildExpiredBanner(context, size),
          ],
          ...
          if (recentList.isNotEmpty) ...[
            _buildSectionHeader(
              ...
              color: AppColors.primaryColor,
              subtitle: 'Booked in the last 7 days',
              isRecent: true,
            ),
            SizedBox(height: size.customHeight(context) * 0.012),
            ...recentList.map(
              (appt) => _AppointmentCard(
                key: ValueKey('recent_${appt.appointmentId}'),
                appt: appt,
                showPayment: isExpiredTab
                    ? appt.isConfirmed
                    : showPayment && !_isExpired(appt),
                showMeetLink: isExpiredTab
                    ? false
                    : showMeetLink && !_isExpired(appt),
                showChildProfileBtn: isExpiredTab
                    ? false
                    : showChildProfileBtn && !_isExpired(appt),
                isExpired: _isExpired(appt),
                isExpiredTab: isExpiredTab,
          ...
          if (olderList.isNotEmpty) ...[
            if (recentList.isNotEmpty)
              _buildSectionDivider(context, size),
            _buildSectionHeader(
              context, size,
              ...
              color: AppColors.textSecondaryColor,
              subtitle:
                  '${olderList.length} appointment${olderList.length == 1 ? '' : 's'}',
              isRecent: false,
            ),
            SizedBox(height: size.customHeight(context) * 0.012),
            ...olderList.map(
              (appt) => _AppointmentCard(
                key: ValueKey('older_${appt.appointmentId}'),
                appt: appt,
                showPayment: isExpiredTab
                    ? appt.isConfirmed
                    : showPayment && !_isExpired(appt),
          ...
          // Edge case: everything is recent
          if (recentList.isNotEmpty && olderList.isEmpty)
            Padding(
              padding: EdgeInsets.only(
                  top: size.customHeight(context) * 0.004),
              child: Center(
                child: Text(
                  'All appointments were booked recently',
                  style: GoogleFonts.poppins(
                      fontSize: 11.5,
                      color: AppColors.textSecondaryColor
                          .withOpacity(0.7)),
  ...
  // ── Section divider ─────────────────────────────────────────
  Widget _buildSectionDivider(BuildContext context, CustomSize size) {
    return Padding(
  ...
  Widget _buildExpiredBanner(BuildContext context, CustomSize size) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border:
            Border.all(color: const Color(0xFFE65100).withOpacity(0.35)),
      ),
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              'These appointments passed without completion.',
              style: GoogleFonts.poppins(
                  fontSize: 12,
                  color: const Color(0xFFE65100),
                  fontWeight: FontWeight.w500),
...
class _AppointmentCard extends StatelessWidget {
  ...
  const _AppointmentCard({
    ...
    required this.isExpired,
    required this.isExpiredTab,
    required this.isRecentHighlight,
    required this.size,
  });
  ...
  _StatusMeta get _meta {
    if (isExpiredTab || (isExpired && (appt.isScheduled || appt.isConfirmed))) {
      return _StatusMeta(
          const Color(0xFFE65100), 'Expired', Icons.timer_off_outlined);
    }
    switch (appt.status.toLowerCase()) {
      case 'confirmed':
        return _StatusMeta(AppColors.primaryColor, 'Confirmed',
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Expired tab: meeting link hint
                  if (isExpiredTab &&
                      appt.meetLink != null &&
                      appt.meetLink!.isNotEmpty) ...[
                    SizedBox(height: size.customHeight(context) * 0.01),
                    _meetLinkExpiredOrangeBanner(context),
                  ],
  ...
  Widget _meetLinkExpiredOrangeBanner(BuildContext context) {
    return Container(
...
// ═══════════════════════════════════════════════════════════════
//   Parent Appointment Detail Screen  (unchanged from original)
// ═══════════════════════════════════════════════════════════════
class ParentAppointmentDetailScreen extends StatefulWidget {
  const ParentAppointmentDetailScreen({super.key});
