...
/// - New: { sa_score, risk_level, severity_percentage, model_type, filename }
/// - Old: { severity_score, max_score, risk_interpretation, bio_markers, filename }
class ExpertSpeechAnalysis {
  // ── New-format fields ──────────────────────────────────────────────────────
  final double? saScore;
  final String? riskLevel;          // e.g. "Moderate"

  // ── Old-format fields ──────────────────────────────────────────────────────
  final int? severityScore;
  final int maxScore;
  final String? riskInterpretation; // e.g. "LOW RISK"
  final ExpertBioMarkers? bioMarkers;
  ...
  factory ExpertSpeechAnalysis.fromJson(Map<String, dynamic> json) {
    return ExpertSpeechAnalysis(
      ...
      riskInterpretation: json['risk_interpretation']?.toString(),
      bioMarkers: json['bio_markers'] != null
          ? ExpertBioMarkers.fromJson(
              json['bio_markers'] as Map<String, dynamic>)
          : null,
  ...
  /// Unified risk label shown in UI regardless of format
  String get displayRiskLabel {
    if (riskLevel != null && riskLevel!.isNotEmpty) return riskLevel!;
    if (riskInterpretation != null && riskInterpretation!.isNotEmpty) {
      return riskInterpretation!;
    }
    return 'UNKNOWN';
  }

  /// True when new-format (sa_score present)
  bool get isNewFormat => saScore != null;

  /// Score shown in the circular progress — sa_score for new, severity_score for old
  double get displayScore => isNewFormat
      ? saScore!
      : (severityScore ?? 0).toDouble();

  /// Max score — new format uses 22 (default), old uses max_score
  int get displayMaxScore => maxScore;

  // ── Risk helpers (work for both formats) ───────────────────────────────────
  bool get isHighRisk {
    final label = displayRiskLabel.toUpperCase();
    return label.contains('HIGH');
  }

  bool get isModerateRisk {
    final label = displayRiskLabel.toUpperCase();
    return label.contains('MODERATE') || label.contains('MEDIUM');
  }
